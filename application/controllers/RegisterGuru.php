...
class RegisterGuru extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('logged_in2')) {
            redirect(base_url() . 'guru/overview');
        }
		$data['category'] = $this->mapel_model->get_category()->result();
    ...
    public function doRegister() {
		...
		$this->form_validation->set_rules('pelajaran', 'Pelajaran', 'required|min_length[3]');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = [
                'name' => $name, 'email' => $email, 'password' => $password,  'pelajaran' => $pelajaran
            ];
			$data2 = [
                'email' => $email, 'kode' => 0
            ];
            //pass the input values to the register model
            $insert_data = $this->register->add_guru($data);
	...
	function get_sub_category(){
        ...
        $data = $this->mapel_model->get_sub_category($category_id)->result();
